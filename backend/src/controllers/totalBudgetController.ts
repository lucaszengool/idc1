...
export const createOrUpdateTotalBudget = async (req: Request, res: Response) => {
  try {
    ...
    if (totalBudget) {
      ...
      totalBudget = await TotalBudget.create({
        budgetYear,
        totalAmount: parseFloat(totalAmount),
        description,
        createdBy,
...
export const getTotalBudget = async (req: Request, res: Response) => {
  try {
...
export const getAllTotalBudgets = async (req: Request, res: Response) => {
  try {
    const totalBudgets = await TotalBudget.findAll({
      order: [['budgetYear', 'DESC']],
    });

    // For each budget, calculate allocated and available amounts
    const budgetsWithStats = await Promise.all(
      totalBudgets.map(async (budget) => {
        const projects = await Project.findAll({
...
export const deleteTotalBudget = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;

    const totalBudget = await TotalBudget.findByPk(id);
    if (!totalBudget) {

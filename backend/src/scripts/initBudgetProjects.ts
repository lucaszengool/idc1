...
interface BudgetProjectData {
  ...
  subProjectName: string; // 子项目名称
  content: string; // 项目内容
  owner: string; // 项目负责人
  budgetAmount: number; // 2026预算（万元）
}

// 26年研发预算评审项目数据
const budgetProjects: BudgetProjectData[] = [
  // IDC-架构研发类别
  {
    category: 'IDC-架构研发',
    ...
    for (const projectData of budgetProjects) {
      try {
        ...
        await Project.create({
          ...
          acceptanceAmount: 0,
          contractOrderNumber: '',
          category: projectData.category as 'IDC-架构研发' | '高校合作' | 'IDC运营-研发',
          subProjectName: projectData.subProjectName,
        });
...
export default initBudgetProjects;
